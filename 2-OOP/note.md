...
## 3.2 继承
- 继承就是一个类可以获得另外一个类中的成员属性和成员方法（满足is—A关系，子类一定要是父类的子集）
- 作用：减少代码，增加代码的复用功能，同时可以设置类与类之间的关系
- 继承与被继承的概念：
    - 呗继承的类叫父类，也叫基类，也叫超类
    - 用于继承的类，叫子类，也叫派生类
    - 继承与被继承一定存在一个 is-a 关系（即This is a pen.）
- 继承的语法，参见oop-2.ipynb
...
- 构造函数
    ...
    - 如果定义了构造函数，则实例化时使用自己的构造，不查找父类构造函数
    - 如果子类没定义，父类的构造函数但参数，则构造对象时的参数应该按父类参数构造
- super
    - super不是关键字，而是一个类
    - super的作用是获取MRO（MethodResolustionOrder）列表中的第一个类
    - super与父类之间没任何实质关系，但通过super可以调用到父类
...
- 构造函数
    ...
    - 构造函数一定要有，如果没有，则自动向上查找，按照MRO顺序查找，直到找到为止
    
## 3.3 多态
- 多态就是同一个对象在不同情况下有不同的状态出现
- 多态不是语法，是一种设计思想
- 多态性：一种调用方式，不同的执行效果
- 多态:同一事物的多种形态，动物分为人类，鸟类，爬行动物类
...
- 使用Mixin实现多继承的时候非常小心
    ...
    - Mixin不能依赖于子类的实现
    - 子类即使没有继承这个Minxin类，也能照样工作，缺少了某个功能
- 优点
...
# 8. 类和对象的三种方法
- 实力方法
    - 需要实例化对象才能使用的方法，使用过程中可能需要借助对象的其它方法完成
- 静态方法
    - 不需要实例化，通过类直接访问
- 类方法
    - 不需要实例化
